    ...
    <div class="addEdit-block">
        <el-form
            ...
            <input id="yonghuId" name="yonghuId" type="hidden">
                <el-col :span="12">
                    <el-form-item class="select" v-if="type!='info'"  label="类型" prop="xinliyishengCollectionTypes">
                        <el-select v-model="ruleForm.xinliyishengCollectionTypes" :disabled="ro.xinliyishengCollectionTypes" placeholder="请选择类型">
                            <el-option
                                v-for="(item,index) in xinliyishengCollectionTypesOptions"
                    ...
                    <div v-else-if="type=='info'">
                        <el-form-item class="input" label="类型" prop="xinliyishengCollectionValue">
                        <el-input v-model="ruleForm.xinliyishengCollectionValue"
...
<script>
    ...
    export default {
        data() {
            return {
                ...
                ro:{
                    ...
                    yonghuId: false,
                    xinliyishengCollectionTypes: false,
                    insertTime: false,
                },
                ruleForm: {
                    xinliyishengId: '',
                    yonghuId: '',
                    xinliyishengCollectionTypes: '',
                    insertTime: '',
                },
                xinliyishengCollectionTypesOptions : [],
                xinliyishengOptions : [],
                yonghuOptions : [],
                rules: {
                   xinliyishengId: [
        ...
        created() {
            ...
            //获取下拉框信息
                this.$http({
                    url:`dictionary/page?page=1&limit=100&sort=&order=&dicCode=xinliyisheng_collection_types`,
                    method: "get"
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.xinliyishengCollectionTypesOptions = data.data.list;
                    }
                });
        ...
        methods: {
            ...
            info(id) {
                let _this =this;
                _this.$http({
                    url: `xinliyishengCollection/info/${id}`,
                    method: 'get'
                }).then(({ data }) => {
                    if (data && data.code === 0) {
            ...
            onSubmit() {
                this.$refs["ruleForm"].validate(valid => {
                    if (valid) {
                        this.$http({
                            url:`xinliyishengCollection/${!this.ruleForm.id ? "save" : "update"}`,
                            method: "post",
                            data: this.ruleForm
                        }).then(({ data }) => {
                            if (data && data.code === 0) {
                                this.$message({
                                    ...
                                    onClose: () => {
                                        ...
                                        this.parent.xinliyishengCollectionCrossAddOrUpdateFlag = false;
                                        this.parent.search();
